...
DB 에서 제거합니다. 둘 다 삭제 API 가 없어서입니다. 몇 번을 돌려도 데모 DB 가 그대로라,
위 "알려진 함정" 의 pytest 문제도 생기지 않습니다. 정리에 실패하면 조용히 넘어가지 않고
`정리` 그룹이 FAIL 로 뜹니다.
...
> 지워질 수 있습니다. 원래 이 DB 는 pytest 와 공유라 단독 사용이 전제입니다(위 "알려진
> 함정"). 삭제 건수를 기대값과 대조하므로, 예상 밖이 섞이면 `정리` 가 FAIL 로 드러납니다.
