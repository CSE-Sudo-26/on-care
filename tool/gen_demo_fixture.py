...
#
# 근력 한 줄은 세트·횟수를 함께 든다(#1276). 유형마다 재는 단위가 달라서, 근력
# 배정이 `10분` 으로만 적히면 회원도 트레이너도 무엇을 몇 번 하라는 것인지
# 화면에서 읽을 수가 없다. 맨몸 운동이라 중량은 비운다 — 적지 않은 값을 `0kg`
# 으로 적으면 트레이너가 정해 준 무게처럼 읽힌다. 같은 운동(`코어 강화 10분`)의
