...
class _MemberHealthProfileDialogState
  ...
  void initState() {
    super.initState();
    _profile = widget.repository.fetchHealthProfile(widget.memberId);
  }
  ...
  Future<void> _save() async {
    ...
    try {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text('회원 정보를 저장하지 못했습니다: $error')));
      ...
      child: FutureBuilder<MemberHealthProfile>(
        ...
        builder: (context, snapshot) {
          if (snapshot.hasError) {
            return Text('회원 정보를 불러오지 못했습니다: ${snapshot.error}');
          }
          if (!snapshot.hasData) {
                    ...
                    children: [
                      ...
                      Expanded(
                        child: TextFormField(
                          ...
                          keyboardType: TextInputType.number,
                          validator: (value) =>
                              _validate(value, min: 0, max: 10080),
                        ),
                      ),
                      const SizedBox(width: 8),
                      Expanded(
                        child: TextFormField(
                          ...
                          keyboardType: TextInputType.number,
                          validator: (value) =>
                              _validate(value, min: 0, max: 100000),
                        ),
                      ),
                    ],
                  ),
