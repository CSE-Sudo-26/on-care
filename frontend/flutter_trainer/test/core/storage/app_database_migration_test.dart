...
void main() {
  test(
    'v12 to v15 adds the daily macro columns and preserves existing rows',
    () async {
      ...
      final executor = NativeDatabase.memory(
        ...
        ''');
          // 실제 v12 DB 에는 이 표가 v1 부터 있다. v14 가 여기에 컬럼을
  ...
  test('v3 to v15 adds macro·주간 계열·취소 기록 columns and preserves rows', () async {
    final executor = NativeDatabase.memory(
      setup: (database) {
        database.execute('''
  ...
  test(
    'v13 to v15 adds the per-meal sugar·date columns and keeps rows',
    () async {
      // 이 표는 그동안 **오늘 하루**만 담아 날짜가 없었다(#1025). 컬럼이 늘어도
      // 있던 끼니는 그대로 읽혀야 한다 — 재시딩 전에도 오늘 화면은 지금까지와
      // 같아야 하기 때문이다.
      final executor = NativeDatabase.memory(
        setup: (database) {
          database.execute('''
      ...
      // 있던 값은 그대로.
      expect(row.meal, '점심');
      expect(row.items, '비빔밥');
      expect(row.calories, 720);
      expect(row.sodiumMg, 1320);
      expect(row.carbsG, 90.5);
      // 새 컬럼은 기본값. 날짜가 빈 행은 날짜로 거르는 조회에 걸리지 않으므로,
      // 재시딩 전에는 기간 뷰의 끼니가 비어 보일 뿐 화면이 깨지지 않는다.
      expect(row.sugarG, 0);
      expect(row.date, '');
    },
  );

  test('v14 to v15 adds the history date column and keeps rows', () async {
    // 표시용 라벨(`8/23 (오늘)`)만으로는 날짜를 견줄 수 없어, 날짜별 기록에
    // 이력을 붙일 수 없었다(#1025). 컬럼이 늘어도 있던 이력은 그대로 읽힌다.
    final executor = NativeDatabase.memory(
      setup: (database) {
        database.execute('''
          CREATE TABLE client_routine_history (
        ...
        database.execute('''
          ...
          VALUES ('seed-history-1', 'seed-client-1', '8/22', 'PT 세션',
                  100, '["스쿼트 3세트 ✓"]')
        ''');
    ...
    final row = await db.select(db.clientRoutineHistory).getSingle();
    final version = await db.customSelect('PRAGMA user_version').getSingle();

    expect(version.read<int>('user_version'), 15);
    expect(row.dateLabel, '8/22');
    expect(row.label, 'PT 세션');
    expect(row.completionRate, 100);
    // 날짜가 빈 행은 날짜로 거르는 조회에 걸리지 않는다 — 재시딩 전에는 그
    // 이력이 날짜별 목록에 붙지 않을 뿐, 화면이 깨지지 않는다.
    expect(row.date, '');
  });
}
